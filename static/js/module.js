...
export function userBtn() {
  ...
  if (userInfo != null) {
    openOverlayBtn.textContent = `Hi, ${
      userInfo.firstname[0].toUpperCase() + userInfo.firstname.slice(1)
    }`;
  }
...
export async function refreshToken() {
  let apiUrl = 'http://localhost:8080/api/auth/refreshtoken';
...
export async function refreshTokenPageRedirection(htmlPage) {
  ...
  try {
    ...
    console.error(
      'Error fetching JSON data (refreshToken redirection):',
      error
    );
  }
}
