...
function manipulateRow(certificate) {
  ...
  setAttributes(moreCertInfo, {
    href: '#certinfo-popup',
    'aria-controls': 'centred-popup',
    class: 'wb-lbx',
    role: 'button',
  });
  moreCertInfo.classList.add(
    'icon-btn',
    'moreCertInfo',
    'glyphicon',
    'glyphicon-info-sign',
...
async function fetctAllCertificate() {
  ...
  let apiUrl = 'http://localhost:8080/api/certificates/user/all';
...
async function sendUrlAndFetchCertificate(userInputUrl) {
  ...
  let apiUrl = 'http://localhost:8080/api/certificates/add';
  ...
  try {
    ...
    if (!response.ok) {
      displayServerErrorMessages(
        'url-form',
        'Something went wrong, please check your URL address.'
      );
      throw data;
    } else {
...
async function fetchDeleteCertificate(certificateId) {
  ...
  let apiUrl = `http://localhost:8080/api/certificates/delete/user/${certificateId}`;
  try {
    const response = await fetch(apiUrl, {
      method: 'DELETE',
      credentials: 'include',
...
async function fetchCertificateById(certificateId) {
  ...
  let apiUrl = `http://localhost:8080/api/certificates/get/${certificateId}`;
  ...
  try {
    ...
    console.error(
      'Error fetching JSON data (get one certificate with id):',
      error
    );
  }
}
...
function populateCertInfo(certificate) {
  ...
  const certSubjectOrganization = document.querySelector(
    '.certSubjectOrganization'
  );
  const certSubjectOrganizationUnit = document.querySelector(
    '.certSubjectOrganizationUnit'
  );

  const certIssuer = document.querySelector('.certIssuerName');
  const certIssuerOrganization = document.querySelector(
    '.certIssuerOrganization'
  );
  const certIssuerOrganizationUnit = document.querySelector(
    '.certIssuerOrganizationUnit'
  );
  ...
  certSubjectOrganization.textContent =
    parsedSubject.O || '<Not Part Of Certificate>';
  certSubjectOrganizationUnit.textContent =
    parsedSubject.OU || '<Not Part Of Certificate>';
  const parsedIssuer = parseCertificateData(certificate.issuer);
  certIssuer.textContent = parsedIssuer.CN || '<Not Part Of Certificate>';
  certIssuerOrganization.textContent =
    parsedIssuer.O || '<Not Part Of Certificate>';
  certIssuerOrganizationUnit.textContent =
    parsedIssuer.OU || '<Not Part Of Certificate>';
